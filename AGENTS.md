...
- 当前默认讨论焦点记录在 `docs/design/mahjong-autobattler-common-origin.md` 第 9 节：**Faust 中哪一种结构应在首次成型后继续存在，并持续把后续输入转化成成长、收益与风险？** 它只是方便新会话续接的检查点，不是强制关卡；用户的新问题、新证据或更有价值的切入点可随时改变讨论顺序。

...
| 3.9 | UI 流程时机钩子（close_wizard/close_begin_guide/close_prompt/sudan_redraw_start/rite_cancel/open_card_info 等，22 个事件） | 六 A5 |

## 批次 4（Low / 打磨）——待做

拖放自动路由到首个满足槽（八 A5）；headless 结算 deferred 先于返卡（八 A6）；auto_begin 不复查 open_condition（八 A7）；金骰成功时不可投入（八 A8）；金币卡注释 2000093→2000029（五）；round<= 退化 Equal 差异（三 A9）；r1 单数值形式（三 A8）；sN 聚合选择器 parent/self/all/enemy/friend（三 A7）；`is` 正形式无 acting 时查槽卡（三 A7）；EventOff 小值批量关闭（六 A6）。

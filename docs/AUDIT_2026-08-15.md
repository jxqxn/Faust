...
> 六（事件系统）见
> [`AUDIT_2026-08-15-event-system.md`](AUDIT_2026-08-15-event-system.md)；
> 七（苏丹经济/回退）见
> [`AUDIT_2026-08-15-sudan-economy.md`](AUDIT_2026-08-15-sudan-economy.md)；
> 八（仪式结算管线）见
> [`AUDIT_2026-08-15-rite-pipeline.md`](AUDIT_2026-08-15-rite-pipeline.md)。
...
## 批次 1（Critical：时间与判定的地基）

| # | 发现 | 报告 | 影响面 | 修复要点 |
|---|------|------|--------|----------|
| 1.1 | round 每天 +1 无条件，仅抽苏丹卡受 HasSudanCard 门控 | 一 A2 | 全部时间语义 | `_begin_round` 与抽卡解耦；round_begin_ba 每天触发；重抽恢复按 round%7 |
| 1.2 | FuncCompare 双字符运算符解析损坏 | 三 A1 | ≈2190 处（>=2001、<=189） | op 改为"表达式末尾最长匹配"（原作 ctor 收预拆分三段佐证） |
| 1.3 | 玩家仪式 start 链缺失：UI 确认即结算 | 一 A1 + 八 A2 | 全部 N 天仪式与苏丹庇护 | UI confirm→CheckConfirm→start/start_round/start_life；OnStop 撤回 life 回滚；结算只在 `_update_rite_instances` |
| 1.4 | timing_rounds 周期冷却重臂缺失 | 六 A1 | 451 个 is_replay 事件 | 按 `player+0x128` 语义：启用时写"下次触发回合"，触发后重臂 |

## 批次 2（High：错误语义反转/缺失）

| # | 发现 | 报告 | 影响面 |
|---|------|------|--------|
| 2.1 | 卡牌生命庇护过窄：应为"任一仪式槽里的一切卡"，不看 start；且卡寿命是通用系统 | 一 A4 | 全部有 life 的卡 |
| 2.2 | `rite` 条件正形式 = 仪式实例存在性（非 rite_end 字典）；含 `rite:1` 哨兵 = 任意仪式 | 三 A3 / 六 A6 | 2191 处 + 10 个死锁事件 |
| 2.3 | `clean.rite` 应删仪式实例而非清卡 | 四 | 若干仪式链 |
| 2.4 | `choose` 应随机抽 N 个（非玩家选择） | 四 | 105 处 |
| 2.5 | `success`/`failed` 应互斥 | 四 | 25 处 |
| 2.6 | have 族计数缺失（s<n>.have/table_have 等） | 三 A5/A6 | 1500+ 处 |
| 2.7 | 槽位求值看不到已放置的其他卡（`rite_state:{}`） | 八 A1 | 引用类槽位条件 |
| 2.8 | 装备标签继承门用错标志位（can_nagative_and_zero→can_inherit） | 五 | 98 个标签 |
| 2.9 | 属性表达式文法缺失（e()/sN.tag/counter.N + 四则分词） | 三 A2 | f:/r: 表达式 |
| 2.10 | 21 种触发时机从未发射（card_born、game_end、counter…） | 六 A3/A5 | 52 个事件 |

## 批次 3（Medium：完整玩法链）

| # | 发现 | 报告 |
|---|------|------|
| 3.1 | 回退上一回合整链缺失（每日双快照、min_round、确认流、LoadRoundEnd 恢复、back_to_round_begin 键） | 七 A1 |
| 3.2 | 骰子重掷（RetryException 重跑整场）路径缺失 | 八 A3 |
| 3.3 | auto_result 仪式 UI 路径应跳过确认 | 八 A4 |
| 3.4 | 重抽失败路径（不回插/不消耗）+ 弃卡实例标签回写池 + 额外重抽计数器 7100008 | 七 A2/A3/A4 |
| 3.5 | counter 默认 op 应 >=（67 处） | 三 A4 |
| 3.6 | 事件日内阻塞模型（Promise 链中途阻塞）vs 克隆 FIFO | 一 A6 |
| 3.7 | think 多分支/引用失真 | 一 A7 |
| 3.8 | game over 保留继续存档（停在 SaveRoundEnd 时刻） | 七 A6 |

## 批次 4（Low / 打磨）

拖放自动路由到首个满足槽（八 A5）；headless 结算 deferred 先于返卡（八 A6）；auto_begin 不复查 open_condition（八 A7）；金骰成功时不可投入（八 A8）；金币卡注释 2000093→2000029（五）；round<= 退化 Equal 差异（三 A9）。

## 不可判定项（不修复，留档）

- OnNextRound 链中 Do* 槽位与 OnRoundEnd 的确切先后（DAT_ 元数据指针不可反查；七 C1、一 A3 存疑点）——保持克隆现顺序，实机反馈驱动。
- 金币 tag 载体映射的数值等价性（三 C5）。
- 快照键字符串字面值（七 C3）——按行为语义自定即可。

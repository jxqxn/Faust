...
| 图鉴/画廊 | `gallery_cards.json` / `gallery_cg.json` + Gallery*Controller 族 | 🟡 标题入口 → `GalleryPanelController`；`GalleryCardPanel.GetCards/ShowCards` 的 `is_show/type/definition/sort` 筛选、六张 `GalleryCardGroup`、可视行 CardNew 实例化已对拍。网格点击按真实链 `GalleryCardItemController.OnPointerClick` 0x547970 → `GalleryCardPanel.ShowCardInfo` 0x549520 → **专用 `ui/gallery_card_info.gd` = `GalleryCardInfo.Show` 0x5465f0**；已落地 3840×2160 真值布局主块、CardNode 名称/标题/正文/标签、`GalleryCardNode.resources/plots` 直读、PlotItem → PlotContent 切换、前后项边界、关闭销毁。剧情遮罩与写点已按 `Show` + `AddShowedGalleryCard` 0x543c80 接到 `Global.showedGalleryCards` 并保存，不再悬空或虚构时机。资源切换路径已按 `ChangeIcon` 0x544210 接线；当前仓库仅有基础卡图，未将语料变体图拷入，缺图时保持原图。`SearchCard` 0x549240 + 闭包 0x55c7f0 = `CardExtensions.GetName(card).Contains`。真值表见 `docs/ui_layout/GalleryCardInfo.md`。仍待：GalleryCardInfo 精灵细件/标签 prefab 的逐像素补齐、结局记录表面、CG 解锁/视觉。 |

...
| 图鉴/画廊 | `gallery_cards.json` / `gallery_cg.json` + Gallery*Controller 族 | 🟡 标题入口 → `GalleryPanelController`；`GalleryCardPanel.GetCards/ShowCards` 的 `is_show/type/definition/sort` 筛选、六张 `GalleryCardGroup`、可视行 CardNew 实例化已对拍。**2026-08-24 纠偏**：网格点击真实链为 `GalleryCardItemController.OnPointerClick` 0x547970 → `GalleryCardPanel.ShowCardInfo` 0x549520 → `GalleryCardInfo.Show` 0x5465f0，绝非普通 `CardInfoNew`；错误直连已撤，`GalleryCardInfo`（资源切换/plot 列表/解锁）转为下一条结构任务，真值表见 `docs/ui_layout/GalleryCardInfo.md`。`SearchCard` 0x549240 + 闭包 0x55c7f0 = InputField 文本按 `CardExtensions.GetName(card).Contains` 过滤，现有 826×90 输入与 110×116 `search.asset` 按此复刻（无自制筛选器）。`GalleryPanelNew/Background` 直绑 `bg_2.asset` → `Texture2D/bg_2.png`；`variable.json`/`ui.json` 直读分类顺序和名称。`Global.overRecord` 原样承载，`overID`/`showedGalleryCards` HashSet 已承载但**未虚构写入时机**。结局记录表面、画廊角色剧情详情、CG 解锁/视觉仍待各自控制器链。 |
...
| global.json 其余字段（gameStatistics/doneEvent/doneRite/showedPrompt/choosedOption/showedGalleryCards/图鉴/升级/任务/overRecord/meta counter） | `save_samples/global.json` 29 字段；承载容器 GlobalState 已落地（backToPrevRound/roundRollback + `overRecord` 原样记录、`overID`/`showedGalleryCards` HashSet 已接） | 中（结局生成、卡牌剧情详情、CG 视觉与其余 Global 域仍待各自控制器链） |

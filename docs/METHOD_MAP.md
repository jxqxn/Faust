...
| ~~结局状态（success/over_reason）~~；cached_event（结构承载） | 前者已落地（2026-08-18 批次 L）：`GameOver.Do → SetGameOver(false, reason)` 同步写 Player.success/@0x79 与 over_reason/@0x7C，默认 reason=int.MinValue；进入 v7 存读档与导入桥。后者已于批次 M 对齐为 Player@0x148 的有序去重提示 id 列表（AddCacheEvent/RemoveCacheEvent）；不是主 UI 队列。2026-08-22 对拍修正：**桌面托盘条目的文案引擎是事件文本 + "在区域中出现了" 定位语句**（语料 event/*.json 含"出现了"的 8+ 文件），但语料**零个 cached_settlement 实例**。2026-08-22 调用方普查（修订）：用户版原游戏（D:\Sultans Game）与语料源**哈希全同**（GameAssembly.dll 与 global-metadata.dat 逐字节一致），反编译产物即该版本；托盘**载入与交互侧在子集内齐全**——`GameController.NoticeCachedEvent 0x5534d0`（枚举队列 → 通知/摇动托盘）、`OnCachedEventClicked 0x5538e0`（点击→结算链 b__0 0x5728d0）、`CachedEventController`（Init/OnPointerClick/Shake）、`StoryNotifyController`；`AddCacheEvent` 仍是仅 EventTrigger.c 两处（cached_settlement 门控）。托盘 UI 批次已解锁：`NoticeCachedEvent` 枚举的队列 = **`GameController.cachedEvents`（dump.cs GameController+0x320 = `Dictionary<int, CachedEventController>`，键 = 事件 id，与 `Player.cached_event` 列表对应）**；托盘文字行来源待区分（StoryNotifyController 的文本条目 vs CachedEventController 的事件条目，用户截图 7 条混合了奖励/追随者/事件到达三类）。**容器几何已确认**：`GameScene.unity` CachedEvents（rect 7782，父 = MainUI 7676）：锚点 (1,0)-(1,0)（右下）、pos (0,680)、pivot (1,0)、sizeDelta 3840×128 —— 全宽 128 高条带位于屏高 ~62.5% 处；条目几何已解（112.5×117 + dialog 图标 + new 红点，见 `docs/ui_layout/CachedEvent.md`）；`CachedEventController.Init 0x527900` **只存字段**（event id→+0x20、对象→+0x28），**不含摆放**；dict 写入点（`cachedEvents[...]=controller`）在反编译中走匿名 Dictionary set_Item 调用，**未能定位**——条目行排列式按"右→左索引序 + 容器 3840×128"登记 🟡，待定位后单点替换。收起态 = 图标行、展开态 = StoryNotify 文字列表 | 小 |

...
| PromptNew 通用事件提示浮层（GameScene MainUI/Prompt；克隆 `_event_overlay` 旧 1280 暗盒待迁） | 未开工（2026-08-22 普查）：`Resources/prefab/PromptNew.prefab` 全真值表 → `docs/ui_layout/PromptNew.md`；`PromptController.Show 0x58a020`（Utils.ProcessPlaceholders 后 set_Text + **ForceRebuildLayoutImmediate**）+ `PromptControllerBase.ShowInternal 0x589890`（Full=config `full` 字段 sprite 装载 0x589d60 TrySetupFull + IconGroup 3 槽 PromptIconController.SetIcon 按内容项填充）+ `OptionNew/OptionNewItem.prefab`（选项行）+ `OptionController 0x576a40` | **核心障碍：OptionBG 高度 = 运行时布局组计算（垂直反转 spacing 100/pad T150 B200 + 子项 preferred 递归），静态不可解**；需原作实机运行时样本（截图/量测）后单点落地 | 中·高（事件主浮层，交互核心面） |

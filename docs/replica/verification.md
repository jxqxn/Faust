...
## main 底座消耗品修复（2026-09-17）

**错误已定位并修复：正常结算混用了超时返卡路径，浴场金币和情报被错误返还。** 方法与完整分流见 [循环正文](loop.md)。本批基于 main 的独立修复分支，创新工作区的未提交研究资料不混入；固定复刻标签不移动。

- 全量 `consumable-fix-full.log`：77脚本、806测试，803通过、3待测、8609断言；无断言失败。无窗口输入法产生显示服务ERROR，`consumable-fix-ime-gpu.log`在GPU下3/3、646断言且无ERROR。待测分别为原作带、BROKER GPU、次日遮罩GPU，次日遮罩 `consumable-fix-mask-gpu.log` 1/1、18断言，另两项见下述原作带与仪式GPU结果。全量启动早于最后的非叠加回收标签与苏丹卡顺序小修，最终消费专项另行复测，不冒充最终树全量重跑。
- 专项最终 GPU 回归：`consumable-fix-rules-gpu-acceptance.log`，6/6、78 断言。包含成功/失败浴场均消费、堆叠余款、回收/非拥有例外、超时返还、清理提示中途保存恢复；1920×1080 与 1280×720 真实鼠标命中并确认结算、保存后状态不返还。
- 既有仪式 UI GPU 回归：`consumable-fix-ui.log`，49/49、494 断言；不能替代新增浴场原作过程对拍。
- 最终原作既存带复跑：`consumable-fix-original-final.log` 与 `consumable-fix-original-final-report.json`；2/2、192断言，结算/读档各54/54、规则随机13/13；验收只覆盖已有第4→5天轨迹，不是浴场或十天新采集。
- 十天教学驱动使用原有安排和种子在修复端重跑，产物 `consumable-fix-ten-day-final.log`：第11天金币35（旧错误版本44），九次浴场收费，投入的2000421秘氛不返还。该脚本是克隆逻辑驱动，随机流未与原作同步，不能作原作标准答案。

仍未闭合：新增浴场原作相同输入/随机带、逐帧表现与关闭至次日完整双端证据。因此本批是源代码背书的底座纠错，**不是完整十天等价或全链还原声明**。文档门禁使用登记的本地证据原字节验证；独立worktree须补齐未进Git的证据，并临时恢复登记文件的原换行字节，验证后撤销检出换行差异，不修改历史证据。旧专项日志保留为修复过程证据，最终日志才是当前边界依据。

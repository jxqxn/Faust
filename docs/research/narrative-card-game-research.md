...
这份研究对原创产线的价值更高，作为方向纠偏和机制评审的查证基线：
...
**这份研究本身不构成命题或具体机制通过的依据**。按 `docs/design/single-character-will-simulation.md §8` 的纪律，每条断言仍需自带 `[已查证]` / `[推断]` / `[印象]` 标签。
...
**这份文档不是**：
- 不是命题或具体机制通过的依据（当前工作方向与评审边界见 `docs/design/single-character-will-simulation.md`）
- 不是完整的叙事卡牌设计理论（Kennedy 的书更权威，但超出本研究范围）
- 不是对苏丹复刻完整度的权威评估（复刻线的权威资料是反编译语料库）

**这份文档是**：
- 方向修正与机制定义期的查证基线，防止后续会话重犯"凭印象下结论"的错误
- 机制评审时断言分类的参考来源
- 一份诚实记录了查证过程中**被推翻的论点**的账本，作为 track record 的一部分

**后续维护**：这份文档遵守 `docs/design/single-character-will-simulation.md §8` 的证据与阶段纪律。任何新增断言必须带 `[已查证]` / `[推断]` / `[印象]` 标签，`[已查证]` 必须配来源 URL。

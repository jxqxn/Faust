...
- 《密教模拟器》组合式叙事引擎与叙事卡牌谱系：[体验原子](../design/cultist-simulator-core-experience-atoms.md)与[叙事卡牌设计谱系研究](narrative-card-game-research.md)，对应 `CS-RULE-001`—`CS-RULE-006`、`CS-EXPL-001`—`CS-EXPL-002`、`CS-GAP-001`。
- 《大骑士物语》图板棋子结构与横版场景表现参照：[图板棋子参照](../design/grand-knights-history-board-pawn-reference.md)，对应 `GK-RULE-001`—`GK-RULE-005`、`GK-EXPL-001`。
- 《这就是警察》事件槽投入与苏丹仪式血统：[事件槽参照](../design/this-is-the-police-event-slot-reference.md)，对应 `TIP-RULE-001`—`TIP-RULE-005`、`TIP-EXPL-001`。
...
- 《圣兽之王》、自动战斗叙事与单机剧情自走棋市场边界：[核心机制参照](../design/unicorn-overlord-autobattle-narrative-reference.md)，对应 `UO-RULE-001`—`UO-RULE-010`、`UO-MKT-001`、`UO-EXPL-001`、`NAB-MKT-001`、`NAB-GAP-001` 与 `NAB-RISK-001`。
...
- 美术方向讨论入库：[美术方向探索](../design/art-direction-exploration.md)、[《苏丹的游戏》美术探索流程还原](sultans-game-art-direction-process.md)、[机器可读候选/测试数据库](data/art-direction-exploration.json)、[完整方法库](data/art-direction-methodology.json)与[未执行评分表](data/art-direction-scorecard.csv)，对应 `FAUST-ART-HYP-001`—`FAUST-ART-DATA-001`。默认路径是“问题空间 → 单个真实作品 → 用户判断 → 反向吸收 → 规则后置”；同题测试与评分表只是在方向出现后的诊断工具。方法库不能定义项目结果；水城、历史媒介与来源转述仍处于待验证状态，不覆盖最高产品方向或现有美术基线。

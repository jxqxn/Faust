...
func test_table_and_sudan_pool_have_positive_and_negative_conditions():
	...
	var ctx := _make_ctx(st, RNG.new(1))
	assert_false(ConditionEval.eval_key("table_have.2000005", 1, ctx), "rite slots are outside Player.cards")
	assert_true(ConditionEval.eval_key("!table_have.2000005", 1, ctx))
	st.add_card_to_hand(2000005, db)
	assert_true(ConditionEval.eval_key("table_have.2000005", 1, ctx), "hidden NPCs still belong to Player.cards")
...
func test_state_trigger_events_queues_matched_ids():
	...
	assert_true(st.pending_operations.is_empty(), "interaction-free events settle silently without entering the display queue")
